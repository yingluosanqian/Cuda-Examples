...
template <typename ProblemShape, typename CtaShape, typename TA,
          typename StrideA, typename SALayout, typename TiledCopyA, typename TB,
          typename StrideB, typename SBLayout, typename TiledCopyB, typename TC,
          typename StrideC, typename SCLayout, typename TiledMma>
__global__ void gemm_device(ProblemShape problem_shape, CtaShape cta_shape,
                            TA const *A, StrideA stride_a, SALayout sA_layout,
                            TiledCopyA tiled_copy_A, TB const *B,
                            StrideB stride_b, SBLayout sB_layout,
                            TiledCopyB tiled_copy_B, TC *C, StrideC stride_c,
                            SCLayout sC_layout, TiledMma tiled_mma) {
  using namespace cute;

  auto mA = make_tensor(make_gmem_ptr(A), select<0, 2>(problem_shape),
                        stride_a); // (m, k)
  auto mB = make_tensor(make_gmem_ptr(B), select<1, 2>(problem_shape),
                        stride_b); // (n, k)
  auto mC = make_tensor(make_gmem_ptr(C), select<0, 1>(problem_shape),
                        stride_c); // (m, n)

  auto cta_coord = make_coord(blockIdx.x, blockIdx.y, _);
  auto gA = local_tile(mA, cta_shape, cta_coord,
                       Step<_1, X, _1>{}); // (bM, bK, k_pipe)
  auto gB = local_tile(mB, cta_shape, cta_coord,
                       Step<X, _1, _1>{}); // (bN, bK, k_pipe)
  ...
  while (k_tile_count > -(K_PIPE_MAX - 1)) {
    ...
    for (int k_block = 0; k_block < K_BLOCK_MAX; k_block++) {
      // gmem -> smem
      if (k_block == 0) {
        copy(tiled_copy_A, tAgA(_, _, _, k_tile_next),
             tAsA(_, _, _, smem_pipe_write));
        copy(tiled_copy_B, tBgB(_, _, _, k_tile_next),
...
void gemm_tn(int m, int n, int k, TA const *A, TB const *B, TC *C) {
  using namespace cute;
...
int main(int argc, char **argv) {
  ...
  gemm(n, m, k, d_A.data().get(), d_B.data().get(), d_C.data().get());
